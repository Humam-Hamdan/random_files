...
colors = ['hsl({}, 50%, 60%)'.format(int(360 * i / len(initial_ks)))
          for i in range(len(initial_ks))]
...
control_card = dbc.Card(
    dbc.CardBody([
        ...
        dbc.Row([
            dbc.Col(dcc.Input(id='custom-k', type='number',
                    placeholder='Add k...', size='small'), width=6),
            dbc.Col(dbc.Button('Add', id='add-k',
                    color='secondary', size='sm'), width=6)
        ], className='mt-2'),
...
app.layout = html.Div([
    dbc.Container([
        dbc.Row(
            dbc.Col(html.H1(id='title', className='text-center my-4 text-secondary'))),
        dbc.Row([
            dbc.Col(control_card, width=3),
            dbc.Col(dcc.Loading(dcc.Graph(id='complex-plot',
                    style={"height": "75vh"})), width=9)
        ])
    ], fluid=True),
...
def update_plot(base, all_ks, selected_ks, line_mode, dot_size, x_step):
    ...
    traces = compute_traces(base, tuple(all_ks), tuple(
        selected_ks), line_on, dot_size, x_step)
